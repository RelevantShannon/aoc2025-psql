WITH banks AS (
    ...
    SELECT DISTINCT input.bank_id
    FROM input),
     max_batteries AS (SELECT (WITH RECURSIVE
                                   bank AS (
                                       ...
                                       SELECT input.bank_id,
                                              input.battery_id,
                                              input.joltage
                                       FROM input
                                       WHERE input.bank_id = banks.bank_id),
                                   -- Get the length of the bank
                                   params AS (SELECT COUNT(*) AS length
                                              FROM bank),
                                       ...
                                       FROM pick
                                           ...
                                           WHERE
                                             ...
                                             -- from the end to guarantee we can pick 12 batteries total
                                               bank.battery_id > pick.min
                                             AND bank.battery_id <= params.length - (12 - pick.step)
                                           ORDER BY bank.joltage DESC,
                                                    bank.battery_id
                               ...
                               SELECT ARRAY_TO_STRING(pick.acc, '') AS max_batteries
                               FROM pick
                               ORDER BY step DESC
                               LIMIT 1)::bigint AS max_batteries
                       FROM banks)
SELECT SUM(max_batteries.max_batteries)
FROM max_batteries
